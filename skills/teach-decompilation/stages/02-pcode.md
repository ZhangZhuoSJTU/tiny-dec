...
## Key Concepts
- **P-code**: A register-transfer language where each operation has an opcode (INT_ADD, LOAD, STORE, BRANCH, etc.), inputs, and an output. Ghidra calls this "p-code" too — tiny-dec's version is simplified but the concept is identical.
...
- **Varnodes**: P-code operands — they reference registers, temporaries, or memory locations by (space, offset, size).
...
```
"Find a LOAD operation. What address is it loading from? Can you trace back through the p-code to figure out what struct field that corresponds to?"
