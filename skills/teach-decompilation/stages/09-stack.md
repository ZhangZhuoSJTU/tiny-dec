...
- **Frame pointer (fp/x8)**: Some functions use x8 as a stable reference point within the frame. Local variables are accessed as `fp - offset` rather than `sp + offset`. The analysis detects when x8 is established as a frame-pointer base.
- **Stack slots**: Named regions within the frame. Each slot has an offset and size, classified into one of four **roles**: `SAVED_REGISTER` (callee-saved registers stored in prologue), `ARGUMENT_HOME` (outgoing ABI argument registers stored in entry block), `LOCAL` (regular local variables), or `UNKNOWN` (insufficient evidence).
- **Symbolic stack-base tracking**: The analysis walks SSA blocks in dominator-tree order, tracking values of the form `frame_top + constant`. It recognizes `INT_ADD` and `INT_SUB` with constants to follow stack pointer adjustments.
...
## Source Files
- `tiny_dec/analysis/stack/transform.py` — `analyze_program_stack()` is the core analysis; the pipeline calls `build_program_stack_facts()` which builds call facts (stage 08) first, then invokes stack recovery. Walks the dominator tree tracking symbolic stack-base values.
- `tiny_dec/analysis/stack/models.py` — `ProgramStackFacts`, `FunctionStackFacts`, `StackSlot`, `StackFrameBase`, `StackAccess`, `StackSlotRole`, `StackBaseKind`, `StackAccessKind`.
- `tiny_dec/analysis/stack/pretty.py` — pretty-printing formatters for stack facts.

Trace how `analyze_function_stack()` seeds x2 at delta 0, follows arithmetic to track the stack pointer, and records LOAD/STORE operations as stack accesses.

## CLI Demonstration

```bash
# Stack recovery for a function with local variables
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_loop_O0_nopie.elf --stage stack --func sum_to_n

# Compare with the struct fixture (more complex frame)
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_struct_O0_nopie.elf --stage stack --func parse_record
```
...
```bash
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_struct_O0_nopie.elf --stage stack --func parse_record
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_struct_O2_nopie.elf --stage stack --func main
```
**Note:** `parse_record` doesn't exist as a separate function in the O2 binary — the compiler inlined it into `main`. This is itself a lesson: optimization eliminates function boundaries, so use `--func main` for the O2 comparison.
