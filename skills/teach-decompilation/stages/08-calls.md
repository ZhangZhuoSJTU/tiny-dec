...
## Key Concepts
- **Calling convention**: RISC-V ILP32 ABI — registers a0-a7 (x10-x17) for the first 8 arguments, a0-a1 (x10-x11) for return values. Extra arguments go on the stack. This is a **fixed** convention (not inferred).
- **Call classification**: Direct calls (known target address), indirect calls (target computed at runtime), and target kind: INTERNAL (within the binary), EXTERNAL (library functions like `malloc`), or UNRESOLVED (unknown target).
- **Call site modeling**: At each CALL/CALLIND instruction, capture SSA values in argument registers, stack argument values at sp-relative offsets, memory versions before/after the call, and return value definitions. The result is a `ModeledCallSite` per call.
- **External signatures**: A curated database of ~35 libc function signatures (malloc, printf, memcpy, etc.) provides parameter counts and no-return flags for known external calls.

## Source Files
- `tiny_dec/analysis/calls/transform.py` — `analyze_program_calls()` is the core analysis; the pipeline calls `build_program_call_facts()` which builds SSA (stage 07) first, then invokes call modeling. Uses a dominator-tree worklist walk to track register bindings and capture call sites.
- `tiny_dec/analysis/calls/models.py` — `ProgramCallFacts`, `FunctionCallFacts`, `ModeledCallSite`, `CallABI`, `CallRegisterValue`, `KnownExternalSignature`.
- `tiny_dec/analysis/calls/signatures.py` — curated external function signature database (~35 libc functions).
- `tiny_dec/analysis/calls/pretty.py` — pretty-printing formatters for call facts.

Read how `_capture_callsite_register_snapshots()` walks the dominator tree maintaining register bindings, and how `_classify_target()` determines internal/external/unresolved.

## CLI Demonstration

```bash
# See call analysis for a multi-function program
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_calls_O0_nopie.elf --stage calls --func main
```
...
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_chain_O0_nopie.elf --stage calls --func main
```
...
```bash
$ poetry run tiny-dec decompile tests/fixtures/bin/fixture_basic_O0_nopie.elf --stage calls --func main
```
