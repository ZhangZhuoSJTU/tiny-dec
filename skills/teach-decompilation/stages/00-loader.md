...
- **Main discovery**: Finding the entry point. The ELF header points to `_start`, but tiny-dec uses heuristics to find the user's `main` function.
...
<details>
<summary>Answer</summary>
Stripped binaries have no symbol table — function names are removed. Even in non-stripped binaries, the ELF entry point is `_start` (the C runtime setup), not `main`. The loader must scan the code near `_start` to find the call to `main` by pattern matching the calling convention.
</details>
