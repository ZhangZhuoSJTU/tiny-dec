...
def test_cli_decompile_stage_variables_outputs_program_snapshot(
    fixture_binary: Callable[[str], Path],
    capsys: pytest.CaptureFixture[str],
) -> None:
    binary = fixture_binary("fixture_struct_O0_nopie")

    exit_code = cli_main(["decompile", str(binary), "--stage", "variables", "--func", "main"])
    output = capsys.readouterr().out

    assert exit_code == 0
    assert "stage: variables" in output
