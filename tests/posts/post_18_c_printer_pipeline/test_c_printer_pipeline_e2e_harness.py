...
def render_c_fixture_snapshots(fixture_bin_dir: Path) -> str:
    rendered: list[str] = []
    ...
    for binary in sorted(fixture_bin_dir.glob("*.elf")):
        ...
        snapshot = render_scheduled_c_program(view, start)
...
def test_c_printer_pipeline_e2e_harness_renders_all_fixture_binaries(
    ...
    combined = render_c_fixture_snapshots(fixture_bin_dir)
    combined_again = render_c_fixture_snapshots(fixture_bin_dir)

    assert combined == combined_again
    assert f"binary: {binaries[0].name}" in combined
    assert "\nc:\n" in combined
    assert "/* scheduled_roots: " in combined
    assert "malloc(32);" in combined
    ...
    assert "call_0x11144_ret = run_steps(&local_40_4, 4, 6);" in combined
