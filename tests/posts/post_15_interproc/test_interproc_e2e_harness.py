...
from tiny_dec.analysis.interproc import build_program_interproc_facts, format_program_interproc_facts
...
def render_interproc_fixture_snapshots(fixture_bin_dir: Path) -> str:
    rendered: list[str] = []
    ...
    for binary in sorted(fixture_bin_dir.glob("*.elf")):
        ...
        snapshot = format_program_interproc_facts(build_program_interproc_facts(view, start))
...
def test_interproc_e2e_harness_renders_all_fixture_binaries(
    ...
    combined = render_interproc_fixture_snapshots(fixture_bin_dir)
    combined_again = render_interproc_fixture_snapshots(fixture_bin_dir)
    ...
    assert "stack+0:4" in combined
    assert "name=local_0_4" in combined
