...
def render_memory_fixture_snapshots(fixture_bin_dir: Path) -> str:
    rendered: list[str] = []
    ...
    for binary in sorted(fixture_bin_dir.glob("*.elf")):
        ...
        snapshot = format_program_memory_facts(build_program_memory_facts(view, start))
...
def test_memory_e2e_harness_renders_all_fixture_binaries(
    ...
    combined = render_memory_fixture_snapshots(fixture_bin_dir)
    combined_again = render_memory_fixture_snapshots(fixture_bin_dir)
