...
def test_cli_decompile_stage_scalar_types_outputs_program_snapshot(
    ...
    exit_code = cli_main(
        ["decompile", str(binary), "--stage", "scalar_types", "--func", "main"]
    )
    output = capsys.readouterr().out

    assert exit_code == 0
    assert "stage: scalar_types" in output
    assert "scalar_types:" in output
