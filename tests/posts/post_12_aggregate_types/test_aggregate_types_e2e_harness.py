...
def render_aggregate_type_fixture_snapshots(fixture_bin_dir: Path) -> str:
    rendered: list[str] = []
    ...
    for binary in sorted(fixture_bin_dir.glob("*.elf")):
        ...
        snapshot = format_program_aggregate_type_facts(
            build_program_aggregate_type_facts(view, start)
        )
...
def test_aggregate_types_e2e_harness_renders_all_fixture_binaries(
    ...
    combined = render_aggregate_type_fixture_snapshots(fixture_bin_dir)
    combined_again = render_aggregate_type_fixture_snapshots(fixture_bin_dir)
