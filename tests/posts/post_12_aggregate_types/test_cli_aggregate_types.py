...
def test_cli_decompile_stage_aggregate_types_outputs_program_snapshot(
    ...
    exit_code = cli_main(
        ["decompile", str(binary), "--stage", "aggregate_types", "--func", "main"]
    )
    output = capsys.readouterr().out

    assert exit_code == 0
    assert "stage: aggregate_types" in output
    assert "aggregate_types:" in output
